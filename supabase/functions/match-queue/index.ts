...
const ENTRY_FEES = new Set([5, 10, 15, 20]);
const cors = {
...
function clampPlayers(value: unknown) {
  const parsed = Number(value);
  if (!Number.isFinite(parsed)) return 6;
  return Math.max(2, Math.min(8, Math.round(parsed)));
}

function parseEntryFee(value: unknown) {
  const parsed = Number(value);
  return ENTRY_FEES.has(parsed) ? parsed : 10;
}
...
serve(async req => {
  ...
  const { user_id, desired = 6, entry_fee = 10 } = await req.json().catch(() => ({}));
  if (!user_id) {
  ...
  if (shouldStart) {
    const roomSize = clampPlayers(queue?.[0]?.desired_players ?? desiredPlayers);
    const players = queue!.slice(0, Math.min(roomSize, queue!.length));
    ...
    const botsNeeded = Math.max(0, Math.min(roomSize, MIN_ROOM_PLAYERS) - players.length);
